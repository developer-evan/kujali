...
export class SelectBudgetPageComponent implements OnInit {
  ...
  private readonly overview = toSignal(this._orgBudgets$$.get(), {
    initialValue: [] as OrgBudgetsOverview,
  });
  private readonly sharedBudgets = toSignal(this._budgets$$.get(), {
    initialValue: [],
  });
  ...
  fieldsFilter(value: (invoice: any) => boolean) {
    return this.allBudgets().budgets.filter(value);
  }
